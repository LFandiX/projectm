...
setup_params = {
    "O2%":    {"value": 21,  "unit": "vol.%", "inc": 1, "min": 21, "max": 100, "format": "{:.0f}", "incricrement": 0},
    "TV":     {"value": 500, "unit": "mL",    "inc": 10,"min": 50, "max": 1500,"format": "{:.0f}", "incricrement": 0},
    "f":      {"value": 12,  "unit": "/min",  "inc": 1, "min": 1,  "max": 100, "format": "{:.0f}", "incricrement": 0},
    "Tinsp":  {"value": 1.70,"unit": "s",     "inc": 0.1,"min": 0.1,"max": 5,   "format": "{:.2f}", "incricrement": 0},
    "PEEP":   {"value": 5,   "unit": "cmH2O", "inc": 1, "min": 0,  "max": 30,  "format": "{:.0f}", "incricrement": 0},
    "Flow":   {"value": 20.0,"unit": "L/min", "inc": 0.5,"min": 5,  "max": 60,  "format": "{:.1f}", "incricrement": 0},
    "F-Trig": {"value": 2.0, "unit": "L/min", "inc": 0.1,"min": 0.5,"max": 10,  "format": "{:.1f}", "incricrement": 0}
}
            ...
            for type, key, action, rect in setup_button_rects:
                ...
                if rect[0] <= x <= rect[0] + rect[2] and rect[1] <= y <= rect[1] + rect[3]:
                    ...
                    if type == "action":
                        ...
                        if action == "inc": p["value"] = min(p["max"], p["value"] + p["inc"])

                        elif action == "dec": p["value"] = max(p["min"], p["value"] - p["inc"])
        ...
        try:
            ...
            if latest_data:
                try:
                    ...
                    p_data["FiO2"] = f"{float(latest_data.get('O2', p_data['FiO2'])):.0f}"
