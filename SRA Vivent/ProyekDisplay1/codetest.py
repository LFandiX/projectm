...
def breathing_wave(t):
    # Complete breathing cycle: 200 frames
    cycle = t % 200
    
    # Define lung capacity levels
    functional_residual = 350      # FRC baseline
    tidal_amplitude = 30          # Normal breathing amplitude
    inspiratory_capacity = 250    # Maximum inspiration
    expiratory_reserve = 420      # Below FRC during forced expiration
    
    if cycle < 40:
        # Normal tidal breathing - small sine waves
        small_wave = tidal_amplitude * math.sin((cycle / 40) * 2 * math.pi)
        return int(functional_residual - small_wave)
    
    elif cycle < 80:
        # Another normal tidal breathing cycle
        small_wave = tidal_amplitude * math.sin(((cycle - 40) / 40) * 2 * math.pi)
        return int(functional_residual - small_wave)
    
    elif cycle < 120:
        # Deep inspiration - rise to inspiratory capacity
        progress = (cycle - 80) / 40.0
        # Smooth S-curve for natural inspiration
        inspiration = (functional_residual - inspiratory_capacity) * (1 + math.sin((progress - 0.5) * math.pi)) / 2
        return int(functional_residual - inspiration)
    
    elif cycle < 160:
        # Deep expiration - drop below FRC to expiratory reserve
        progress = (cycle - 120) / 40.0
        # Smooth curve for expiration
        expiration = (expiratory_reserve - functional_residual) * (1 + math.sin((progress - 0.5) * math.pi)) / 2
        return int(functional_residual + expiration)
    
    else:
        # Return to FRC and start next small tidal wave
        progress = (cycle - 160) / 40.0
        # Return smoothly to baseline and begin small oscillation
        return_to_baseline = (expiratory_reserve - functional_residual) * (1 - progress)
        small_wave = tidal_amplitude * math.sin(progress * 2 * math.pi) * progress  # Fade in the small wave
        return int(functional_residual + return_to_baseline - small_wave)
