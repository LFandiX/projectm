...
#4. mencari jam daring juan       
    ...
    data_daring_juan_perday = {}
    tanggal_daring_juan = []
    for index in data_daring:
        if index[3]== "Juan:": #cari yang namanya juan
            if index[0] in data_daring_juan_perday: #cari apakah tanggal nya udah ada di dict tapi khusus juan
                continue #hanya ambil jam pertama kali dia on di hari  index[0]
            else:
                data_daring_juan_perday[index[0]] = [index[1]] 
                tanggal_daring_juan.append(index[0]) 
        else:
            pass
    
    #Jam Daring Juan
    waktu_daring,tanggal_daring = count_to_list(data_daring_juan_perday)
    #mencari jam 
    integer_waktu_daring =[]
    for waktu_juan in waktu_daring:
        for waktu in waktu_juan:
            jam = waktu.split(":")[0]
            integer_waktu_daring.append(int(jam))
    ...
    fig4.bar (tanggal_daring_juan,integer_waktu_daring)
    plt.xlabel ("Tanggal")
    plt.ylabel ("Jam")
    plt.title ("Jam Daring Pertama Juan")
    plt.show()